...
def check_and_perform_auto_reset():
    ...
    if now < this_monday_reset_time and now.weekday() == 0: # 월요일이고 아직 6시 전이면
         this_monday_reset_time -= timedelta(days=7) # 지난주 월요일 6시 기준
...
with col1:
    ...
    if st.button("캐릭터 추가"):
        ...
        if new_char_name and new_char_name not in [c["name"] for c in characters]:
            characters.append({
                "name": new_char_name,
    ...
    if len(characters) > 0:
        if st.button("캐릭터 수정"):
            ...
            if selected_char_name_for_modify:
                 new_char_name = st.text_input(f"{selected_char_name_for_modify}의 새 이름 입력:", value=selected_char_name_for_modify)
                 if new_char_name and new_char_name != selected_char_name_for_modify and new_char_name not in [c["name"] for c in characters]:
                    for char in characters:
                     ...
                     st.info("이름 변경이 없습니다.")
                 elif new_char_name in [c["name"] for c in characters]:
    ...
    if len(characters) > 0:
        if st.button("캐릭터 삭제"):
            ...
            if selected_char_name_for_delete:
                ...
                if characters:
                    st.session_state.selected_char = characters[0]["name"] # 첫 번째 캐릭터 선택
                else:
...
if len(characters) > 0:
    ...
    selected_char_name = st.selectbox("캐릭터 선택:", char_names, key="selected_char")
    ...
    if selected_char_data:
        ...
        # 길드 출석 (공용)
        shared_guild_checked = st.checkbox("길드 출석 (모든 캐릭터 공유)", value=shared_tasks["길드 출석"] == 1, key="shared_guild")
        ...
        # 나머지 일일 숙제
        for task, count in list(daily_tasks.items()): # list()로 복사하여 순회 중 수정 가능하게 함
            if task == "길드 출석":
                continue # 이미 위에서 처리함
            if task == "망령의 탑":
        ...
        for task, count in list(weekly_tasks.items()): # list()로 복사하여 순회 중 수정 가능하게 함
             st.markdown(f"**{task}:** 남은 횟수 {count}")
             if count > 0:
